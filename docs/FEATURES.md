...
| `Agent Monitor` | With Jev configured, periodically evaluates active agent work (10 seconds by default; configurable from 5 to 300 seconds). Shows `Thinking` while output streams and detects completed turns and input requests. The `Thinking` indicator also works without Jev. |
