...
- **右ペイン**: Files、Review、Browserを切り替えて使えます。中央のターミナル配置はそのまま保たれます。
...
`Ctrl+Shift+P` でコマンドパレットを開きます。1つの入力欄で操作、ワークスペース、Space、ターミナルペインを検索できます。**All**、**Actions**、**Locations** で検索対象を絞り、矢印キーで移動、`Enter` で選択、`Escape` で閉じます。
...
設定では外観とUIサイズ、ターミナルのフォントとスクロールバック、シェル、Browser、Attention、ショートカット、Jev連携、アップデートを調整できます。テーマはSystem、Light、Dark、Graphiteの4種類です。
