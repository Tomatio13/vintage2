...
Vintage is a desktop-first and web-compatible AI workspace. The interface should feel calm, dense, and operational rather than decorative.
...
Code and diff syntax highlighting uses dedicated theme tokens rather than semantic feedback colors:
...
Comments may use `--color-foreground-subtlest`. Keep these roles readable in every shipped theme; do not use success, warning, or destructive colors to distinguish syntax categories.
...
Vintage should use restrained depth. Layer primarily through background contrast, borders, and radius before relying on heavy shadows.
