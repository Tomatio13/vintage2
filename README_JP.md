...
- **ファイルプレビュー**: Filesから画像、HTML、PDF、音声、動画、Markdown、JSON、CSV/TSV、一般的なソース・テキストファイルを表示
...
5. **ファイルのプレビュー**: 右側ペインの `Files` を開き、ファイルをダブルクリックすると中央ペイン右側にプレビューが開きます。対応形式と操作は[ファイルプレビュー](#ファイルプレビュー)を参照してください。
...
- **Markdown** はワークスペース内の画像や表を表示でき、`Preview` と `Source` を切り替えられます。
- **JSON** は整形表示または生ソース、**CSV/TSV** は検索できる表またはソース表示に切り替えられます。
- **HTML** はスクリプトと外部リソースをブロックした隔離プレビューで表示し、ソース表示にも切り替えられます。
- **画像**にはズーム操作があります。**PDF**は組み込みビューアーで表示し、音声・動画には再生コントロールがあります。
- 一般的なソース・テキストファイルはシンタックス表示に対応します。検索、折り返し、コピー、再読み込みの操作は形式や表示モードに応じて利用できます。
- Office文書やZIPアーカイブなど、未対応形式はOSのアプリで開けます。

テキストプレビューは最大1 MB、先頭10,000行までです。画像は最大10 MB、HTMLプレビューとそのCSSリソースは最大5 MBです。
...
- **安全なファイル読み取り**: パストラバーサル防止、シンボリックリンクによる意図しない外部参照の遮断、通常ファイル以外の読み取り拒否を徹底しています（テキストは最大1 MB、画像は最大10 MB、HTMLプレビューとそのCSSリソースは最大5 MB）。
